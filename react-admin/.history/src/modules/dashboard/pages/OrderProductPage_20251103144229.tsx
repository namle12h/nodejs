...
  // ... (Giữ nguyên logic kiểm tra đầu vào và xử lý status)

  if (!apiInvoices || apiInvoices.length === 0) return [];
  return apiInvoices.map(invoice => {
    
    // Ví dụ: Giả định có trường createdAt
...
    }
    ...
    // --------------------------------------------------------
    const customerIdFallback = invoice.customer?.id ?? 'N/A'; // Lấy ID an toàn
    const customerName = invoice.customer?.fullName ?? `Khách hàng #${customerIdFallback}`;
    const customerEmail = invoice.customer?.email ?? `khachhang${customerIdFallback}@email.com`;
    
    // --------------------------------------------------------
    // 🎯 2. SỬA LỖI VÀ LẤY TÊN SẢN PHẨM/DỊCH VỤ
    // --------------------------------------------------------
    const primaryItem = invoice.items?.[0]; // Lấy item đầu tiên an toàn
    let productName: string = "Không có mục nào";
...
    return {
        // Dùng txnRef nếu có, nếu không dùng ID
      id: invoice.txnRef ? `${invoice.txnRef}` : `INV-${invoice.id}`,
      type: invoice.appointmentId !== null ? 'Appointment' : 'Product',
      customer: customerName, // ✅ Đã sửa: dùng Tên
      email: customerEmail, // ✅ Đã sửa: dùng Email
      product: productName, // ✅ Đã sửa: dùng Tên Dịch vụ/Sản phẩm
