...
import { useAllServices, useAllProducts } from '../../../shared/services/serviceApi';
...
// Định nghĩa kiểu dữ liệu cho Map
...
export const useNameLookupMaps = () => {
  ...
  const serviceNameMap: NameMap = useMemo(() => {
    ...
    return serviceData.reduce((acc, service) => {
      acc[service.id] = service.name; // Giả định mỗi object có trường 'id' và 'name'
      return acc;
    }, {} as NameMap);
  }, [serviceData]);
  ...
  const productNameMap: NameMap = useMemo(() => {
    ...
    return productData.reduce((acc, product) => {
      acc[product.id] = product.name; // Giả định mỗi object có trường 'id' và 'name'
      return acc;
    }, {} as NameMap);
  }, [productData]);
...
const mapApiInvoicesToOrders = (
    ...
    serviceNameMap: NameMap,  // 👈 Tham số mới
    productNameMap: NameMap   // 👈 Tham số mới
): Order[] => {
    if (!apiInvoices || apiInvoices.length === 0) return [];

    return apiInvoices.map(invoice => {
        // 1. Xử lý Ngày giờ và Trạng thái (GIỮ NGUYÊN)
        ...
        if (primaryItem) {
            ...
            // Ưu tiên 2: CHỈ CÓ ID (Sử dụng Map tra cứu chính xác từ DB)
            else if (primaryItem.serviceId) {
                const name = serviceNameMap[primaryItem.serviceId] ?? `#${primaryItem.serviceId}`;
                productName = `Dịch vụ: ${name}`;
            } else if (primaryItem.productId) {
                const name = productNameMap[primaryItem.productId] ?? `#${primaryItem.productId}`;
        ...
        return {
            ...
            email: customerEmail,
            product: productName, // ✅ Đã có tên chính xác
