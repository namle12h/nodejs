...
    }
    ...
    // Ưu tiên lấy từ receiverName (dữ liệu có sẵn), sau đó là CustomerDTO
    // --------------------------------------------------------
    const customerIdFallback = invoice.customer?.id ?? 'N/A'; 

    const customerName = invoice.receiverName // 🥇 Ưu tiên: Receiver Name (từ đơn hàng)
        // 🥈 Thứ cấp: Tên từ Customer DTO
        ?? invoice.customer?.receiverName 
        // 🥉 Cuối cùng: ID
        ?? `Khách hàng #${customerIdFallback}`; 
        
    const customerEmail = invoice.receiverPhone // 🥇 Ưu tiên: Phone (dùng tạm email)
        // 🥈 Thứ cấp: Email từ Customer DTO
        ?? invoice.customer?.email 
        // 🥉 Cuối cùng: Email mặc định
        ?? `khachhang${customerIdFallback}@email.com`; 
    
    // --------------------------------------------------------
    // 🎯 2. SỬA LỖI VÀ LẤY TÊN SẢN PHẨM/DỊCH VỤ (An toàn Object)
    // --------------------------------------------------------
    const primaryItem = invoice.items?.[0]; // items có thể null
    let productName: string = "Không có mục nào";

    if (primaryItem) {
        if (primaryItem.service) {
            // ✅ Lấy TÊN dịch vụ từ Object DTO (nếu có)
            productName = `Dịch vụ: ${primaryItem.service.name ?? `#${primaryItem.service.id}`}`;
        } else if (primaryItem.product) {
            // ✅ Lấy TÊN sản phẩm từ Object DTO (nếu có)
