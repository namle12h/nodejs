...
const mapApiInvoicesToOrders = (apiInvoices: ApiInvoice[]): Order[] => {
  ...
  return apiInvoices.map(invoice => {
    ...
    return {
      ...
      customer: `Khách hàng #${invoice.customerId}`,
      email: `khachhang${invoice.customerId}@email.com`,
      product: invoice.items[0] ? (invoice.items[0].serviceId ? `Dịch vụ #${invoice.items[0].serviceId}` : `Sản phẩm #${invoice.items[0].productId}`) : "Nhiều mục",
...
const OrderProductPage: React.FC = () => {
  ...
  const { data: apiInvoices, isLoading, isError } = useGetAllInvoices();
