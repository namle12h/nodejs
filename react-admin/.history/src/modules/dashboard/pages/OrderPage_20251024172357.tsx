...
const OrderPage = ({ orderData }: { orderData: AppointmentRecord }) => {
    ...
    //             // customerId: orderData.customerId || 70, // nếu có sẵn thì lấy từ orderData
    ...
    //                     quantity: 1,
    //                     unitPrice: orderData.service?.price,
    //                 },
    //                 // dịch vụ thêm
    //                 ...extraServices.map((s) => ({
    //                     serviceId: s.id,
    ...
    //                 })),
    //             ],
    //             paymentMethod: payment, // "cash", "wallet", etc.
    //             amountPaid: amountPaid, // 💵 số tiền khách đưa
    //             notes: notes,            // 📝 ghi chú đặc biệt

    //         };

    //         console.log("🧾 Payload gửi backend:", payload);
    //         console.log("🟢 Đã click nút xác nhận thanh toán!");
    ...
    const handleConfirmPayment = async () => {
        try {
            ...
            // 🔹 Nếu backend trả về paymentUrl → redirect sang VNPay
            ...
            // 🔹 Nếu không có paymentUrl (VD: thanh toán tiền mặt)
            message.success("Thanh toán thành công!");
            localStorage.removeItem("extra_services");
            localStorage.removeItem("extra_products");
