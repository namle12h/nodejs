...
const mapApiInvoicesToOrders = (
    ...
    return apiInvoices.map(invoice => {
        ...
        // ... (Phần 1. Xử lý Ngày giờ và Trạng thái - GIỮ NGUYÊN)
        ...
        // ... (Phần 2. Xử lý Tên Khách hàng - GIỮ NGUYÊN)
        const customerIdFallback = invoice.customerId ?? 'N/A'; // Sửa từ customer?.id sang customerId nếu chỉ có ID

        const customerName = invoice.receiverName
            ?? (invoice.customerId as any)?.receiverName // Giả định customer DTO có thể là any
            ?? `Khách hàng #${customerIdFallback}`;

        const customerEmail = invoice.receiverPhone
            ?? (invoice.customerId as any)?.email
            ?? `khachhang${customerIdFallback}@email.com`;

        // --------------------------------------------------------
        // 🔥 LOGIC SỬA LỖI VÀ LẤY TÊN SẢN PHẨM/DỊCH VỤ (Dùng Map tra cứu)
        // --------------------------------------------------------
        ...
        if (primaryItem) {
            ...
            // Ưu tiên 1: Có Object DTO (Nếu API trả về đầy đủ, DTO là Service/Product)
            ...
            // Ưu tiên 2: CHỈ CÓ ID (Dùng Map tra cứu từ file useNameLookupMaps)
            // Lưu ý: DTO của bạn KHÔNG CÓ serviceId/productId, nhưng vì API cũ trả về ID,
            // chúng ta sẽ giả định thêm (hoặc kiểm tra) trường ID trên item.
            // Nếu API chỉ trả về service/product DTO, thì phần này KHÔNG cần thiết.
            // *Để tương thích với API cũ (chỉ có ID) & API mới (có DTO)*, ta sẽ thêm logic tra cứu an toàn:
            
            // Nếu không có DTO, ta thử tra cứu qua Map (dựa trên giả định API cũ có ID)
            const serviceIdFromItem: number = (primaryItem as any).serviceId;
            const productIdFromItem: number = (primaryItem as any).productId;

            if (!productName && serviceIdFromItem) { 
                const name = serviceNameMap[serviceIdFromItem] ?? `#${serviceIdFromItem}`;
                productName = `Dịch vụ: ${name}`;
            } else if (!productName && productIdFromItem) {
                const name = productNameMap[productIdFromItem] ?? `#${productIdFromItem}`;
        ...
        return {
            ...
            email: customerEmail,
            product: productName, // ✅ Tên dịch vụ/sản phẩm đã được tra cứu
...
// --------------------------------------------------------------------------------
// Đặt hàm này vào file OrderProductPage.tsx (thay thế hàm mapApiInvoicesToOrders cũ)
// --------------------------------------------------------------------------------
...
const OrderProductPage: React.FC = () => {
  ...
  const fetchedOrders: Order[] = useMemo(() => {
    ...
    return mapApiInvoicesToOrders(apiInvoices, serviceNameMap, productNameMap);
  }, [apiInvoices, serviceNameMap, productNameMap, isLoadingLookup]);

  if (isLoading) { // ✅ Đã bao gồm cả lookup loading
    return (
