...
const mapApiInvoicesToOrders = (apiInvoices: ApiInvoice[]): Order[] => {
  ...
  return apiInvoices.map(invoice => {
    ...
    return {
      ...
      customer: `Khách hàng #${invoice.customer.id}`,
      email: `khachhang${invoice.customer.id}@email.com`,
      product: invoice.items[0] ? (invoice.items[0].service?.id ? `Dịch vụ #${invoice.items[0].service.id}` : `Sản phẩm #${invoice.items[0].product.id}`) : "Nhiều mục",
