...
const AppointmentManager = () => {
    ...
    // Tổng tất cả
    const totalAppointmentsAll = data?.length ?? 0;

    // Lịch hẹn hôm nay
    const todayAppointments = data?.content.filter((a: any) =>
        dayjs(a.startAt).format("YYYY-MM-DD") === today
    )?.length ?? 0;

    // Đã xác nhận
    const confirmed = data?.content.filter((a: any) => a.status === "Confirmed")?.length ?? 0;

    // Đang chờ
    const pending = data?.content.filter((a: any) => a.status === "Pending" || a.status === "Waiting")?.length ?? 0;

    // Hoàn thành
    const completed = data?.content.filter((a: any) => a.status === "Completed")?.length ?? 0;

    // Hủy
    const cancelled = data?.content.filter((a: any) => a.status === "Cancelled")?.length ?? 0;

    // Tỷ lệ hủy (%)
    const cancelRate = totalAppointmentsAll
        ? ((cancelled / totalAppointmentsAll) * 100).toFixed(1)
        : 0;
        ...
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            <div className="grid grid-cols-6 gap-4 mb-6">
                <Card className="text-center shadow rounded-2xl">
                    <p className="font-bold text-xl">{totalAppointmentsAll}</p>
