...
const mapApiInvoicesToOrders = (
  ...
  return apiInvoices.map(invoice => {
    ...
    const preferredCustomerName = invoice.receiverName
      // Ưu tiên 2: Tên từ DTO object (nếu API mới trả về)
      ?? invoice.customer?.receiverName
      // Ưu tiên 3: Tạo tên từ ID
      ?? `Khách hàng #${customerIdFallback}`;

    const customerName = preferredCustomerName; // Sử dụng tên đã chọn
