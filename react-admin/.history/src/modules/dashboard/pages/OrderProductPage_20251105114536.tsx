...
const mapApiInvoicesToOrders = (
    ...
    return apiInvoices.map(invoice => {
        ...
        // 2. Xử lý Tên Khách hàng
        const customerIdFallback = invoice.customerId ?? 'N/A';

        // Sử dụng Optional Chaining an toàn sau khi đã thêm trường 'customer' vào ApiInvoice
        ...
        if (primaryItem) {
            ...
            // Ưu tiên 2: Không có DTO, sử dụng Map tra cứu ID (tương thích ngược với API cũ)
            // LƯU Ý: Dùng 'as any' để truy cập serviceId/productId vì chúng có thể không tồn tại trên DTO mới
            ...
            if (!productName || productName === "Mục không xác định") { // Chỉ tra cứu nếu chưa xác định tên
                 if (serviceId) { 
