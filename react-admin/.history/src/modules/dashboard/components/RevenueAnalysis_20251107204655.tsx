...
// Dữ liệu cho các loại phân tích (Đã lược bỏ CustomerActivity/Transaction History không dùng nữa)
            ...
            <div className="flex items-start mb-6">
                ...
                <div className="flex-1">
                    <div className="flex justify-between items-center mb-4">
                        ...
                        {/* Dropdown So với kỳ trước (đã được thay thế bằng buttons ở component cha) */}
...
export default function RevenueAnalysis() {
    ...
    const [comparisonPeriod, setComparisonPeriod] = useState<'ky_truoc' | 'nam_truoc'>('ky_truoc');
    ...
    const comparisonOptions = [
        { key: 'ky_truoc', label: 'So với kỳ trước' },
        { key: 'nam_truoc', label: 'So với năm trước' },
    ];


    return (
        // Xóa grid lg:grid-cols-3 để chỉ còn 1 cột duy nhất
        <div className="space-y-6"> 

            {/* Vùng Dropdown/Button điều khiển */}
            <div className="flex justify-start items-center space-x-2">
                
                {/* Nút Loại dữ liệu (Khách hàng / Doanh thu...) */}
                {categoryOptions.map(option => (
                    <button
                        key={option.key}
                        onClick={() => setDataCategory(option.key as 'revenue' | 'customer' | 'appointment' | 'product')}
                        className={`
                            px-4 py-2 text-sm font-medium rounded-lg shadow-sm border transition-colors
                            ${dataCategory === option.key
                ...
                {/* Nút Loại so sánh (So với kỳ trước / So với năm trước) */}
                {comparisonOptions.map(option => (
                    <button
                        key={option.key}
                        onClick={() => setComparisonPeriod(option.key as 'ky_truoc' | 'nam_truoc')}
                        className={`
                            px-4 py-2 text-sm font-medium rounded-lg shadow-sm border transition-colors
                            ${comparisonPeriod === option.key
            ...
            {/* 2. Phần Thừa đã bị xóa (CustomerAnalysisBlock) */}
        </div>
    );
}