...
const TrendCard = ({ item }: { item: TrendCardItem }) => {
    ...
    return (
        <div className="bg-white p-5 rounded-xl shadow-md border border-gray-100 flex flex-col justify-between">
            ...
            <h4 className="text-2xl font-bold text-left text-gray-900 mb-1">{item.currentValue}</h4>
            <p className="text-xs text-left text-gray-500">Trước: {item.prevValue}</p>
        </div>
    );
};
...
const RevenueComparisonBlock = () => (
    <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
        ...
        <div className="mt-8 border-t pt-4">
            <h4 className="text-lg font-semibold text-left text-gray-800 mb-4">Chi Tiết So Sánh</h4>
                ...
                return (
                    <div key={index} className="flex justify-between items-center py-3 border-b last:border-b-0 border-gray-100">
                        ...
                        <div className="flex-1">
                            <p className="font-medium text-left text-gray-900">{item.period}</p>
                            <p className="text-sm text-left text-gray-500">{item.currentValue} vs {item.prevValue}</p>
                        </div>
        ...
        {/* Nhận xét tự động */}
        <div className="mt-6 p-4 bg-blue-50/50 rounded-lg border text-left border-blue-100">
