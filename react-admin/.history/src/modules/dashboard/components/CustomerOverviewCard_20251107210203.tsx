...
// --- SVG Icons (Thay thế cho các icon từ thư viện nếu không muốn cài đặt) ---
...
const UserGroupIcon = (props:any) => (
  <svg {...props} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor">
    <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.501a12.001 12.001 0 002.35 1.488 15.004 15.004 0 0011.666 0 12.001 12.001 0 002.35-1.488M12 14.25a6.75 6.75 0 00-6.75 6.75h13.5A6.75 6.75 0 0012 14.25z" />
  </svg>
);
...
const CustomerOverviewCard = () => {
  const CustomerItem = ({ icon: Icon, title, count, color }: CustomerOverviewCardProps) => (
...
// --- Recent Customers Card Component ---
const RecentCustomersCard = () => {
  const CustomerListItem = ({ avatar, name, time, revenue, visits }) => (
  ...
  const customers = [
    { name: 'Nguyễn Thị Hoa', time: '2 ngày trước', revenue: '2.4Mđ', visits: 12, avatar: 'https://via.placeholder.com/40/FBBF24/FFFFFF?text=H' },
    { name: 'Trần Văn Nam', time: '1 tuần trước', revenue: '1.8Mđ', visits: 8, avatar: 'https://via.placeholder.com/40/60A5FA/FFFFFF?text=N' },
    { name: 'Lê Thị Minh', time: '3 ngày trước', revenue: '3.2Mđ', visits: 15, avatar: 'https://via.placeholder.com/40/FBBF24/FFFFFF?text=M' },
    { name: 'Phạm Minh Đức', time: '5 ngày trước', revenue: '1.5Mđ', visits: 6, avatar: 'https://via.placeholder.com/40/60A5FA/FFFFFF?text=D' },
  ];
...
// --- Service Stats Card Component ---
const ServiceStatsCard = () => {
  const ServiceItem = ({ iconColor, title, amount, count, percentage }) => (
    <div className="flex items-center justify-between py-3 border-b border-gray-100 last:border-b-0">
      ...
      <div className="text-right w-1/4">
        <p className="text-sm font-semibold" style={{ color: iconColor.replace('bg-', 'text-') }}>{percentage}</p>
      </div>
    </div>
  );

  const services = [
    { title: 'Massage Thư Giãn', amount: '22.5Mđ', count: 45, percentage: '35%', color: 'bg-teal-500' },
    { title: 'Chăm Sóc Da Mặt', amount: '19.2Mđ', count: 32, percentage: '28%', color: 'bg-purple-500' },
    { title: 'Tắm Trắng Body', amount: '16.8Mđ', count: 28, percentage: '22%', color: 'bg-blue-500' },
    { title: 'Nail & Spa', amount: '9.0Mđ', count: 18, percentage: '15%', color: 'bg-pink-500' },
  ];
  const totalRevenue = '67.5Mđ';
