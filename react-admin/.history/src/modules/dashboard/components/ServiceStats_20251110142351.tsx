...
export default function ServiceStats() {
 ...
 const [showAllServices, setShowAllServices] = useState(false);
    ...
    // Nếu dữ liệu từ API không có trường `color`, chúng ta sẽ gán màu thủ công
       const dataWithColors = topServices.map((item: Service, index:number) => ({
        ...item,
    ...
    const remainingTotalAppointments = remainingServices.reduce((sum: number, service: Service) => sum + service.totalAppointments, 0);
            ...
            <div className="mt-3 text-left space-y-3">
                ...
                {remainingServices.length > 0 && (
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            <span className="w-3 h-3 rounded-full" style={{ background: "#888" }}></span>
                            <div>
                                <p className="font-medium">Khác</p>
                                <p className="text-gray-500 text-sm">{remainingTotalAppointments} lượt đặt</p>
                            </div>
                        </div>

                        <div className="text-right">
                            <p className="font-semibold">
                                {((remainingTotalAppointments / total) * 100).toFixed(1)}%
                            </p>
                            <p className="text-gray-500 text-sm">Doanh thu không xác định</p>
                        </div>
                    </div>
                )}
            </div>
