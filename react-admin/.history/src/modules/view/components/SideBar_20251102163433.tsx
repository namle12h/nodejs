...
import { User, Lock, List, MapPin, Bell, LogOut, ShoppingCart } from "lucide-react";
...
export default function Sidebar() {
  ...
  const menu = [
    ...
    {
      icon: <List size={18} />, label: "Lịch sử đơn hàng", path: "/profile/orders", children: [
        { icon: <ShoppingCart size={16} />, label: "Đơn hàng sản phẩm", path: "/profile/orders/products" },
        { icon: <Calendar size={16} />, label: "Đơn hàng dịch vụ", path: "/profile/orders/services" },
      ],
    },
      ...
      <nav className="space-y-1">
        {menu.map((item, i) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-2 rounded-md text-left transition-all ${location.pathname === item.path
