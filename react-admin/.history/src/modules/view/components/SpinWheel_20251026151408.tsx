...
const SpinWheel = () => {
  ...
  // ✅ Thứ tự đúng với vòng quay hiện tại (bắt đầu từ 12h, theo chiều kim đồng hồ)
  const prizes = ["Voucher 50k", "Voucher 100k", "2% giảm giá", "5% giảm giá", "10% giảm giá", "20% giảm giá"];
  ...
  // ⚙️ Tính lại chỉ số phần thưởng (xoay theo chiều kim đồng hồ, điểm 0 nằm ở 12h)
  const prizeIndex =
    Math.floor((normalizedRotation + segmentAngle / 2) / segmentAngle) %
    prizes.length;

  // Lật lại thứ tự cho đúng chiều vòng
  const correctedIndex =
    (prizes.length - prizeIndex - 1 + prizes.length) % prizes.length;

  const wonPrize = prizes[correctedIndex];
