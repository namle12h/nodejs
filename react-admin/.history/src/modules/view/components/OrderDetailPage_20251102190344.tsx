...
import { useOrders } from "../../../shared/services/orderApi" // 👈 import hook bạn đã tạo ở file trên
...
export default function OrderList() {
  ...
  const { data: orders, isLoading, isError } = useOrders(false); // false = không phải admin
                ...
                {order.orderItems?.map((p: any, i: number) => (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <div>
                        <p className="font-medium text-gray-800">{p.product?.name}</p>
                        <p className="text-sm text-gray-500">
                          {p.product?.brand || "Không rõ"} • SL: {p.quantity}
                        </p>
                      </div>
                    </div>
