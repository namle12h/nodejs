...
import { useCreateOrder } from "../../../shared/api/orderApi";
...
export default function CheckoutPage() {
    ...
    const handleCreateOrder = async (finalOrderData: any, isCOD: boolean = false) => {
        ...
        // ⭐️ FIX: LẤY TOKEN MỚI NHẤT NGAY TỪ ZUSTAND STORE
        // Đảm bảo bạn đã import useAuthStore ở đầu file.
        const { token: authStoreToken } = useAuthStore.getState();
        const currentToken = authStoreToken?.accessToken;

        if (!currentToken) { // 👈 SỬ DỤNG currentToken (từ getState)
        ...
        try {
            const response = await axios.post(
                ...
                finalOrderData,
                // ⭐️ SỬ DỤNG currentToken (từ getState)
            ...
            if (errorMsg.startsWith("PENDING:")) {
                // ... (Logic xử lý Modal PENDING giữ nguyên, sử dụng currentToken)
            ...
            // Lỗi chung (bao gồm 403 Forbidden)
