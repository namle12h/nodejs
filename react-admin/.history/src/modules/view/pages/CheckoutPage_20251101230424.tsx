...
export default function CheckoutPage() {
    ...
    // ... (Các imports, state, và các hàm khác giữ nguyên)

    // ✅ SỬA LOGIC CỦA HÀM handleCreateOrder
    const handleCreateOrder = async (finalOrderData: any, isCOD: boolean = false) => {
        setLoadingSubmit(true);
        
        // ⭐️ FIX: LẤY TOKEN MỚI NHẤT TRỰC TIẾP TỪ ZUSTAND STORE
        const { token: authStoreToken } = useAuthStore.getState(); 
        const latestToken = authStoreToken?.accessToken;
        
        if (!latestToken) { // 👈 KIỂM TRA latestToken
...
                finalOrderData,
                // ⭐️ SỬ DỤNG latestToken Ở ĐÂY
                { headers: { 'Authorization': `Bearer ${latestToken}` } } 
            );
            // ... (Phần còn lại của logic API giữ nguyên)
            
        } catch (error: any) {
            // ... (Logic xử lý lỗi PENDING giữ nguyên)

            // Cần sửa logic PENDING để sử dụng latestToken trong onOk
...
                                // ⭐️ SỬ DỤNG latestToken Ở ĐÂY
                                { headers: { 'Authorization': `Bearer ${latestToken}` } } // 👈 FIX: SỬ DỤNG latestToken
                            );
                            // ...
                        } catch (e) {
                            // ...
