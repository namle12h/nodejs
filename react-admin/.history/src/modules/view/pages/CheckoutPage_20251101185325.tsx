...
import "@ant-design/v5-patch-for-react-19";
import Header from "../../../shared/components/Header";
import axios from "axios"; // 👈 Thêm axios để gọi API thanh toán
...
// 🆕 Cập nhật User interface
type User = { 
    id?: number; 
    name?: string | null;
    email?: string | null; // 👈 Thêm email
    phone?: string | null; // 👈 Thêm phone
};


const useCart = () => ({
    // Giả lập giỏ hàng có dữ liệu
    cart: [
...
// 🆕 Giả lập useAuth trả về thông tin người dùng
const useAuth = () => ({
    user: { 
        id: 123, 
...
export default function CheckoutPage() {
    ...
    // 🔴 Bỏ orderIdFromServer, OrderDataForPayment vì sẽ xử lý trực tiếp (chỉ giữ lại cho CODPayment)
    const [orderDataForPayment, setOrderDataForPayment] = useState<any>(null); 
    const [loadingSubmit, setLoadingSubmit] = useState<boolean>(false);
    ...
    useEffect(() => {
        ...
        if (orderId && vnpResponseCode !== null) {
            ...
            if (vnpResponseCode === "00" && vnpTransactionStatus === "00") {
                ...
                // Có thể điều hướng về lại trang checkout sau 3s
                // setTimeout(() => { /*...*/ }, 3000); 
            }
        }
    }, [isClient, clearCart]);
        ...
        try {
            ...
            const res = await axios.post(
                "http://localhost:8080/api/vnpay/create_payment", // ⚠️ Endpoint tạo link VNPay
                {
                    ...
                    // Gửi lại đường dẫn chính xác của trang checkout để VNPay callback
    ...
    // ✅ Gọi API tạo đơn hàng (Đã cập nhật để chỉ trả về kết quả)
    const handleCreateOrder = async (finalOrderData: any, isCOD: boolean = false) => {
        setLoadingSubmit(true);
        try {
            // ⚠️ Đổi endpoint sang /api/orders (endpoint giả định)
            const response = await fetch(
            ...
            if (response.ok) {
                ...
                if (isCOD) {
                    ...
                    // Xử lý VNPay/Card: trả về dữ liệu đơn hàng để tạo thanh toán
                    return { success: true, orderId: orderId, data };
                }
            ...
            // Chỉ bỏ loading nếu không có redirect (chỉ dùng cho COD)
        ...
        form
            ...
            .then(async (values) => { // ❗ Thêm async
                ...
                const orderData = {
                    ...
                    userfullname: user?.name,
                    paymentMethod: paymentMethod,
                    // Trạng thái ban đầu: pending cho cả 2 phương thức
                    status: "pending", 
                    orderItems: (cart || []).map((item: CartItem) => ({
                        productId: item.id,
                        quantity: item.quantity,
                ...
                if (paymentMethod === "cod") {
                    // Lưu dữ liệu để hiển thị Modal xác nhận COD
                    setOrderDataForPayment(orderData);
                    setShowCODPayment(true); 
                }
                // 2. Xử lý Thanh toán qua cổng (Card/VNPay)
                    ...
                    setLoadingSubmit(false); // Bỏ loading nếu gọi API thất bại
    ...
    return (
        <div className="min-h-screen bg-gray-50">
                ...
                <Form form={form} layout="vertical">
                    <div className="flex flex-wrap -mx-4">
                        ...
                        <div className="w-full lg:w-2/3 px-4 mb-8">
                            ...
                            <ShippingInfo
                                form={form}
                                user={user} {/* 👈 Truyền thông tin user vào ShippingInfo */}
                                cities={cities}
                                setCities={setCities}
                                districts={districts}
                                setDistricts={setDistricts}
            ...
            {showCODPayment && orderDataForPayment && (
                ...
                //     onCreateOrder={(data) => handleCreateOrder(data, true)} 
                // />
            )}
        </div>
    );
