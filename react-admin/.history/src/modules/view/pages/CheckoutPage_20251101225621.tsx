...
export default function CheckoutPage() {
    ...
    // ✅ Gọi API tạo đơn hàng (đã điều chỉnh để trả về kết quả và xử lý PENDING)
                    ...
                    onOk: async () => {
                        try {
                            ...
                            const res = await axios.get(
                                ...
                                // ⭐️ THÊM TOKEN VÀO HEADER CHO LẦN GỌI NÀY
                                { headers: { 'Authorization': `Bearer ${token}` } }
                            );
                            if (res.data?.paymentUrl) {
                            ...
                            <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
                                <OrderSummary
                                    orderItems={selectedOrderItems.map(item => ({
                                        ...
                                        quantity: item.quantity,
                                        price: item.price,
                                        // Map imageUrl thành image cho OrderSummary (giải quyết lỗi ảnh)
                                        image: item.imageUrl,
                                    }))}
