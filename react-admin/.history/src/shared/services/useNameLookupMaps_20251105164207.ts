...
const productNameMap: NameMap = useMemo(() => {
    ...
    return productData.reduce((acc, product: ProductDTO) => {
        // ✅ Đảm bảo ID được lưu với tên an toàn
        // Ưu tiên tên thật, nếu không có, dùng ID (ví dụ: Sản phẩm #43)
        const nameToUse = product.name ?? `Sản phẩm #${product.id}`; 
        
        acc[product.id] = nameToUse; // Lưu tên đã xử lý
        return acc;
    }, {} as NameMap);
}, [productData]);
// Trong file useNameLookupMaps.ts
