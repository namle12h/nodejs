...
export default function Header() {
    ...
    useEffect(() => {
        ...
        if (firstLoad.current) {
            ...
            prevIds.current = notifications.map((n) => n.id);
            return;
        }

        const currentIds = notifications.map((n) => n.id);

        // 🔸 Chỉ phát âm thanh nếu có ID mới chưa có trong danh sách trước đó
        const hasNew =
            prevIds.current.length > 0 &&
            currentIds.some((id) => !prevIds.current.includes(id));
