...
const mapApiInvoicesToOrders = (
  ...
  return apiInvoices.map(invoice => {
    ...
    if (primaryItem) {
      ...
      // Ưu tiên 1: Lấy tên từ DTO nhúng (Nếu có)
      ...
      if (serviceId) {
        const name = serviceNameMap[serviceId] ?? `#${serviceId}`; // Tra cứu bằng ID
        productName = `Dịch vụ: ${name}`;
      } else if (productId) {
        const name = productNameMap[productId] ?? `#${productId}`; // Tra cứu bằng ID
...
const OrderProductPage: React.FC = () => {
  ...
  } = useGetAllInvoice(page, 10, "createdAt");
  ...
  const calculatedStats = useMemo(() => {
    // ✅ Nếu không có data hoặc content rỗng
    if (!apiInvoices || !apiInvoices.content || apiInvoices.content.length === 0) {
      return {
        totalOrders: 0,
        todayOrders: 0,
        totalRevenue: 0,
    ...
    apiInvoices.content.forEach((invoice) => {
      totalOrders++;
      ...
      try {
        const invoiceDate = new Date(invoice.createdAt).toLocaleDateString("vi-VN");
  ...
  // ✅ Sửa lỗi: Đã đặt useMemo bên trong component, sử dụng apiInvoices

  // 🔄 ĐỊNH NGHĨA MẢNG THỐNG KÊ ĐỘNG
  const dynamicStatData: StatCardProps[] = [
    {
      title: 'Tổng đơn hàng',
      value: calculatedStats.totalOrders.toLocaleString('vi-VN'),
      change: '+12%', // Giữ nguyên giả định
      isPositive: true,
      icon: '🛒'
    },
    {
      title: 'Đơn hàng hôm nay',
      value: calculatedStats.todayOrders.toLocaleString('vi-VN'),
      change: calculatedStats.todayOrders > 0 ? '+100%' : '0%',
      isPositive: calculatedStats.todayOrders > 0,
      icon: '🗓️'
    },
    {
      title: 'Tổng Doanh thu',
      value: `${calculatedStats.totalRevenue.toLocaleString('vi-VN')}₫`,
      change: '+15%',
      isPositive: true,
      icon: '💸'
    },
    {
      title: 'Đơn chờ xử lý',
      value: calculatedStats.pendingOrders.toLocaleString('vi-VN'),
      change: '-5%',
      isPositive: false,
      icon: '⏱️'
    },
  ...
  const fetchedOrders: Order[] = useMemo(() => {
    // Nếu chưa có data hoặc đang đợi lookup
    if (!apiInvoices || !apiInvoices.content || isLoadingLookup) return [];

    return mapApiInvoicesToOrders(
      apiInvoices.content,    // ✅ chỉ truyền mảng content
      serviceNameMap,
      productNameMap,
      customerNameMap
    );
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="flex justify-between items-center mt-4 px-2 py-3">
          <button
            disabled={page === 0}
          ...
          <span className="text-sm font-medium text-gray-700">
            Trang {page + 1} / {apiInvoices?.totalPages || 1}
          </span>

          <button
            disabled={page + 1 >= (apiInvoices?.totalPages || 1)}
            onClick={() => setPage(page + 1)}
            className="px-4 py-2 bg-gray-100 rounded disabled:opacity-50 hover:bg-gray-200"
          >
            Trang sau
